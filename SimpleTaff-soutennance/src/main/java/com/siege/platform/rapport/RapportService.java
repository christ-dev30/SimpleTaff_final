...
public class RapportService {
    ...
    public Map<String, Object> genererRapportAgent(String mois, UUID agentId) {
        YearMonth ym = YearMonth.parse(mois);
        LocalDate debutMois = ym.atDay(1);
        LocalDate finMois = ym.atEndOfMonth();
        LocalDateTime debutDateTime = debutMois.atStartOfDay();
        ...
        Map<String, Object> report = new LinkedHashMap<>();
        report.put("titre", "RAPPORT INDIVIDUEL AGENT - " + mois);
        report.put("dateGeneration", LocalDate.now());
        report.put("period", mois);
        ...
        List<Mission> missionsMois = allMissions.stream()
                .filter(ms -> ms.getPlanningDebut() != null && !ms.getPlanningDebut().isBefore(debutMois) && !ms.getPlanningDebut().isAfter(finMois))
                .filter(ms -> ms.getAgent() != null && ms.getAgent().getId().equals(agentId))
                .toList();
        Map<String, Object> secMissions = new LinkedHashMap<>();
        secMissions.put("total_missions", missionsMois.size());
        secMissions.put("prevues", missionsMois.stream().filter(m -> "PREVUE".equalsIgnoreCase(m.getStatut())).count());
    ...
    public byte[] exportToExcel(Map<String, Object> report) {
        ...
        if (report.containsKey("par_agent")) {
            Map<String, List<Map<String, Object>>> parAgent = (Map<String, List<Map<String, Object>>>) report.get("par_agent");
            sb.append(borderLine).append("\n");
            sb.append("MODULE : POINTAGES PAR AGENT\n");
            sb.append("Total Agents;").append(report.getOrDefault("total_agents", 0)).append("\n");
            sb.append("Nombre d'Entrées Totales;").append(report.getOrDefault("nombre_entrees", 0)).append("\n");
            sb.append(borderLine).append("\n");
            sb.append("Agent Terrain;Date;Heure d'Entrée;Heure de Sortie;Site de Travail\n");

            for (Map.Entry<String, List<Map<String, Object>>> entry : parAgent.entrySet()) {
        ...
        if (report.containsKey("lignes_facturation")) {
            Map<String, Map<String, Object>> facturation = (Map<String, Map<String, Object>>) report.get("lignes_facturation");
            sb.append(borderLine).append("\n");
            sb.append("MODULE : FACTURATION\n");
            sb.append("Total Heures;").append(report.getOrDefault("total_heures", 0)).append("\n");
            sb.append("Montant Total Estimé;").append(report.getOrDefault("montant_total_estime", 0)).append("\n");
            sb.append(borderLine).append("\n");
            sb.append("Affectation (ID);Agent Terrain;Heures Totales\n");

            for (Map.Entry<String, Map<String, Object>> entry : facturation.entrySet()) {
                Map<String, Object> details = entry.getValue();
                sb.append(escapeCsv(entry.getKey())).append(";")
                  .append(escapeCsv(details.get("agent"))).append(";")
                  .append(escapeCsv(details.get("heures"))).append("\n");
            }
            sb.append("\n");
        }
