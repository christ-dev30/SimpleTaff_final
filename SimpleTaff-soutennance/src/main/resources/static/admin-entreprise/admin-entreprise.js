...
window.zoomQr = function (agentNom, codeQr) {
  document.getElementById("qrModalTitle").textContent =
    `Badge QR Code — ${agentNom}`;
  document.getElementById("qrModalImg").src =
    `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${codeQr}`;
...
async function loadOverview() {
  try {
    ...
    if (window.allAffectations || allAgents) {
      ...
      const sortedJobs = Object.keys(countByJob)
        .sort((a, b) => countByJob[b] - countByJob[a])
        .slice(0, 5);
      if (sortedJobs.length > 0) {
        const maxCount = countByJob[sortedJobs[0]];
        const totalJobs =
          Object.values(countByJob).reduce((sum, val) => sum + val, 0) || 1;
        ...
        if (barsContainer && labelsContainer) {
          barsContainer.innerHTML = sortedJobs
            .map((job, idx) => {
          ...
          labelsContainer.innerHTML = sortedJobs
            .map((job) => {
    ...
    try {
      ...
      if (tbody) {
        if (!affectations || affectations.length === 0) {
          tbody.innerHTML =
            '<tr><td colspan="5" class="px-6 py-6 text-center text-slate-400">Aucune affectation récente.</td></tr>';
        } else {
          tbody.innerHTML = affectations
            .slice(0, 6)
            .map((a) => {
              const initials = (a.agentNom || "A")
                .split(" ")
                .map((n) => n[0])
                .join("")
                .substring(0, 2)
      ...
      console.error("Error overview affectations:", e);
    }
    ...
    try {
      ...
      if (tbodyPt) {
        if (!pointages || pointages.length === 0) {
          tbodyPt.innerHTML =
            '<tr><td colspan="4" class="px-6 py-6 text-center text-slate-400">Aucun pointage aujourd\'hui.</td></tr>';
        } else {
          tbodyPt.innerHTML = pointages
            .slice(0, 6)
            .map((p) => {
              ...
              const timeStr = p.dateHeureEntree
                ? new Date(p.dateHeureEntree).toLocaleTimeString([], {
                    hour: "2-digit",
                    minute: "2-digit",
                  })
                : "—";
      ...
      const totalExpected =
        stats.totalAffectationsActives > 0 ? stats.totalAffectationsActives : 1;
      const agentsPresentsCount = new Set(
        pointages.map((p) => p.agentId || p.agentNom || p.id),
      ).size;
      const tauxPresence = Math.min(
        100,
        Math.round((agentsPresentsCount / totalExpected) * 100),
      );
...
window.addCityToZone = function (cityName) {
  ...
  if (
    !window.selectedZoneCities.some(
...
window.removeCityFromZone = function (cityName) {
  window.selectedZoneCities = window.selectedZoneCities.filter(
    (c) => c !== cityName,
  );
  renderSelectedZoneCities();
};
...
window.renderSelectedZoneCities = function () {
  ...
  if (window.selectedZoneCities.length === 0) {
    container.innerHTML =
      '<span class="text-xs text-slate-400 w-full text-center py-1" id="noCitiesPlaceholder">Aucune ville ajoutée</span>';
    return;
  }

  container.innerHTML = window.selectedZoneCities
    .map((city) => {
...
document
  ...
  ?.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      const perimetreVal =
        document.getElementById("zonePerimetre").value.trim() ||
        selectedVilles.join(", ");
...
async function loadCatalog() {
  try {
    ...
    if (emploiTbody) {
      emploiTbody.innerHTML =
        emplois
          ...
          .join("") ||
        '<tr><td colspan="3" class="p-3 text-center text-slate-400">Catalogue vide.</td></tr>';
    }

    const paramEmploiTbody = document.getElementById("paramEmploisTableBody");
    if (paramEmploiTbody) {
      paramEmploiTbody.innerHTML =
        emplois
          .map(
            (e) => `
                        <tr class="hover:bg-slate-50/50 transition-colors">
    ...
    if (agentZoneSelect) {
      agentZoneSelect.innerHTML =
        '<option value="">Sélectionner sa zone</option>' +
        zones.map((z) => `<option value="${z.id}">${z.nom}</option>`).join("");
    }

    const mAgentZoneSelect = document.getElementById("mAgentZoneSelect");
    if (mAgentZoneSelect) {
      mAgentZoneSelect.innerHTML =
        '<option value="">Sélectionner sa zone...</option>' +
        zones.map((z) => `<option value="${z.id}">${z.nom}</option>`).join("");
    }
...
window.filterAgents = function () {
  const searchVal = document
    .getElementById("searchAgentInput")
    .value.toLowerCase()
    .trim();
  const zoneId = document.getElementById("filterAgentZoneSelect").value;

  const activeList = allAgents.filter(
    (a) =>
      a.statut !== "EN_ATTENTE_CONTRAT_SIGNE" &&
  ...
  const filtered = activeList.filter((a) => {
    ...
    const matchesZone =
      ...
      a.zoneId === zoneId ||
      a.zoneNom ===
        document.getElementById("filterAgentZoneSelect").options[
          document.getElementById("filterAgentZoneSelect").selectedIndex
        ].text;
  ...
  if (agentsTbody) {
    agentsTbody.innerHTML =
      filtered
        ...
        .join("") ||
      '<tr><td colspan="8" class="p-3 text-center text-slate-400">Aucun agent enregistré.</td></tr>';
  }

  const pendingList = allAgents.filter(
    (a) =>
      a.statut === "EN_ATTENTE_CONTRAT_SIGNE" ||
      a.statut === "EN_ATTENTE_FINALISATION_ADMIN",
  );
...
window.deleteAgent = async function (id) {
  if (
    !confirm(
      "Attention: La suppression de cet agent est irréversible et supprimera également toutes ses pièces justificatives, ses contrats, ses affectations de matériel et ses cartes de pointage. Voulez-vous continuer ?",
    )
  ) {
    return;
  }
...
async function loadModalZones() {
  ...
  const zones =
    window.allZones || (await apiFetch("/organisation/zones")) || [];
...
const uploadAndBindFile = (inputId, statusId, hiddenInputId) => {
  ...
  newInput.addEventListener("change", async (e) => {
    ...
    const enrollmentInputs = [
      "mAgentPhotoFile",
      "file-CNI_Passeport",
      "file-Extrait_Naissance",
      "file-Casier_Judiciaire",
      "file-CV",
      "file-Diplomes",
      "file-Certificats_Travail",
      "file-Permis_Conduire",
      "file-Attestations_Diverses",
      "file-Photo_Identite",
    ];

    if (enrollmentInputs.includes(inputId)) {
      const currentInputSize = enroleFileSizes[inputId] || 0;
      const totalBytesExcludingCurrent =
        Object.values(enroleFileSizes).reduce((a, b) => a + b, 0) -
        currentInputSize;
      const potentialTotal = totalBytesExcludingCurrent + file.size;

      if (potentialTotal > 13 * 1024 * 1024) {
        alert(
          "La limite cumulée de 13 Mo pour l'ensemble des fichiers est dépassée. Le téléversement est bloqué.",
        );
        newInput.value = "";
        delete enroleFileSizes[inputId];
        updateCumulativeSizeIndicator();
    ...
    if (statusSpan) {
      ...
      statusSpan.className =
        "text-xs mt-1 block text-amber-500 font-bold animate-pulse";
    }
    try {
...
uploadAndBindFile(
  "file-CNI_Passeport",
  "status-CNI_Passeport",
  "url-CNI_Passeport",
);
uploadAndBindFile(
  "file-Extrait_Naissance",
  "status-Extrait_Naissance",
  "url-Extrait_Naissance",
);
uploadAndBindFile(
  "file-Casier_Judiciaire",
  "status-Casier_Judiciaire",
  "url-Casier_Judiciaire",
);
uploadAndBindFile("file-CV", "status-CV", "url-CV");
uploadAndBindFile("file-Diplomes", "status-Diplomes", "url-Diplomes");
uploadAndBindFile(
  "file-Certificats_Travail",
  "status-Certificats_Travail",
  "url-Certificats_Travail",
);
uploadAndBindFile(
  "file-Permis_Conduire",
  "status-Permis_Conduire",
  "url-Permis_Conduire",
);
uploadAndBindFile(
  "file-Attestations_Diverses",
  "status-Attestations_Diverses",
  "url-Attestations_Diverses",
);
uploadAndBindFile(
  "file-Photo_Identite",
  "status-Photo_Identite",
  "url-Photo_Identite",
);
...
document
  .getElementById("modalAddAgentForm")
  ?.addEventListener("submit", async (e) => {
    e.preventDefault();
    ...
    const payload = {
      ...
      situationMatrimoniale: document.getElementById(
        "mAgentSituationMatrimoniale",
      ).value,
      ...
      telephoneSecondaire: document.getElementById("mAgentTelephoneSecondaire")
        .value,
      ...
      contactUrgenceTelephone: document.getElementById("mAgentUrgenceTelephone")
        .value,
    ...
    try {
      ...
      if (emploiId) {
        const opt =
          document.getElementById("mAgentEmploiSelect").options[
            document.getElementById("mAgentEmploiSelect").selectedIndex
          ];
      ...
      alert(
        "Dossier Agent complet créé avec succès ! Le contrat est en attente de signature.",
      );
      ...
      document.getElementById("mAgentPhotoStatus").textContent =
...
window.switchPersonnelSubTab = function (tab) {
  ...
  if (tab === "actifs") {
    btnActifs.className =
      "px-5 py-2 rounded-xl text-sm font-bold transition-all duration-300 shadow-sm bg-white text-indigo-700 ring-1 ring-slate-200";
    btnEnrol.className =
      "px-5 py-2 rounded-xl text-sm font-medium transition-all duration-300 text-slate-500 hover:text-slate-700 hover:bg-slate-200/50 flex items-center gap-1.5";
    secActifs.classList.remove("hidden");
    secEnrol.classList.add("hidden");
  } else {
    btnActifs.className =
      "px-5 py-2 rounded-xl text-sm font-medium transition-all duration-300 text-slate-500 hover:text-slate-700 hover:bg-slate-200/50";
    btnEnrol.className =
      "px-5 py-2 rounded-xl text-sm font-bold transition-all duration-300 shadow-sm bg-white text-indigo-700 ring-1 ring-slate-200 flex items-center gap-1.5";
...
window.renderEnrolementAgents = function (list) {
  const searchVal =
    document
      .getElementById("searchEnrolementInput")
      ?.value?.toLowerCase()
      ?.trim() || "";
  ...
  if (tbody) {
    tbody.innerHTML =
      filtered
        ...
        .join("") ||
      '<tr><td colspan="6" class="p-3 text-center text-slate-400">Aucun agent en cours d\'enrôlement.</td></tr>';
  }
};
...
window.openFinalizeEnrollmentModal = async function (agentId) {
  ...
  document.getElementById("feDateDebut").value = new Date()
    .toISOString()
    .split("T")[0];
  ...
  if (agent && agent.createdByCoordonnateurNomPrenom) {
    ...
    const creatorEmail = agent.createdByCoordonnateurEmail
      ? ` (${agent.createdByCoordonnateurEmail})`
      : "";
    document.getElementById("feCreatorNameEmail").textContent =
  ...
  try {
    ...
    if (feSiteSelect) {
      feSiteSelect.innerHTML =
        '<option value="">Ne pas affecter immédiatement</option>' +
        sites
          .map(
            (s) =>
              `<option value="${s.id}">${s.nom} (${s.structureNom})</option>`,
          )
          .join("");
    }
  } catch (e) {
...
window.openFinaliserAgentAdminModal = async function (agentId) {
  ...
  document.getElementById("finaliserAdminNomPrenom").textContent =
    `${agent.nom || ""} ${agent.prenom || ""}`;
  document.getElementById("finaliserAdminContact").textContent =
    agent.contact || "Non renseigné";
  document.getElementById("finaliserAdminZone").textContent =
    agent.zoneNom || "Non renseignée";
  const creatorName = agent.createdByCoordonnateurNomPrenom || "Inconnu";
  const creatorEmail = agent.createdByCoordonnateurEmail
    ? ` (${agent.createdByCoordonnateurEmail})`
    : "";
  document.getElementById("finaliserAdminCreator").textContent =
    `${creatorName}${creatorEmail}`;

  document
    .getElementById("finaliserAgentAdminModal")
...
window.submitFinaliserAgentAdmin = async function () {
  ...
  try {
    const response = await apiFetch(`/agents/${agentId}/finaliser`, {
      method: "POST",
    });
    if (window.showToast)
      window.showToast(
        "Création finalisée avec succès. L'agent est maintenant en attente de contrat.",
        "success",
      );
...
document
  .getElementById("finalizeEnrollmentForm")
  ?.addEventListener("submit", async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      showToast(
        "L'enrôlement a été finalisé ! L'agent est désormais ACTIF.",
        "success",
      );
    ...
    if (select) {
      ...
      select.innerHTML =
        ...
        emplois
          .map(
            (e) =>
              `<option value="${e.id}" data-salaire="${e.salaireBrutReference}" data-libelle="${e.libelle}">${e.libelle} (${e.salaireBrutReference} FCFA)</option>`,
          )
          .join("");
      if (currentVal) {
...
window.updateContractPreview = function () {
  ...
  const dateNaissance = document.getElementById("mAgentDateNaissance").value
    ? new Date(
        document.getElementById("mAgentDateNaissance").value,
      ).toLocaleDateString("fr-FR")
    : "[DATE DE NAISSANCE]";
  const lieuNaissance =
    document.getElementById("mAgentLieuNaissance").value ||
    "[LIEU DE NAISSANCE]";
  const nationalite =
    document.getElementById("mAgentNationalite").value || "[NATIONALITÉ]";
...
window.updateFolderContractPreview = function () {
  ...
  const salaire =
    document.getElementById("fcSalaireBase").value || "[SALAIRE DE BASE]";
  ...
  const dateDebut = dateDebutInput
    ? new Date(dateDebutInput).toLocaleDateString("fr-FR")
    : "[DATE DÉBUT]";
  const dateFinInput = document.getElementById("fcDateFin").value;
  const dateFin = dateFinInput
    ? new Date(dateFinInput).toLocaleDateString("fr-FR")
    : null;

  const structureSelect = document.getElementById("fcStructureCliente");
  const structureName =
    structureSelect && structureSelect.selectedIndex > 0
      ? structureSelect.options[structureSelect.selectedIndex].text
      : "[STRUCTURE CLIENTE]";
  ...
  const dateNaissance = agent.dateNaissance
...
window.printFolderContract = function () {
  ...
  const w = window.open();
  w.document.write(
    `<pre style="font-family: serif; font-size: 14px; white-space: pre-wrap; padding: 40px; line-height: 1.6; max-width: 800px; margin: auto; text-align: justify;">${text}</pre>`,
  );
  w.document.close();
  w.print();
};
...
window.openAgentFolder = async function (agentId) {
  ...
  document.getElementById("folderAgentSub").textContent =
    `Matricule : ${a.matricule || "En cours"}`;

  document.getElementById("fAgentGenre").textContent = a.genre || "—";
  document.getElementById("fAgentNaissance").textContent = a.dateNaissance
    ? `${a.dateNaissance} (Lieu: ${a.lieuNaissance || "—"})`
    : "—";
  document.getElementById("fAgentNationalite").textContent =
    a.nationalite || "—";
  document.getElementById("fAgentMatrimoniale").textContent =
  ...
  document.getElementById("fAgentTelSec").textContent =
    a.telephoneSecondaire || "—";
  document.getElementById("fAgentEmail").textContent = a.email || "—";
  document.getElementById("fAgentLocalisation").textContent =
    `${a.commune || "—"} / ${a.ville || "—"}`;
  document.getElementById("fAgentAdresse").textContent = a.adresse || "—";

  document.getElementById("fUrgenceNom").textContent =
    a.contactUrgenceNom || "—";
  document.getElementById("fUrgenceTel").textContent =
    a.contactUrgenceTelephone || "—";
  document.getElementById("fUrgenceLien").textContent =
    a.contactUrgenceLien || "—";
...
async function loadFolderDocs() {
  ...
  try {
    ...
    if (!pieces || pieces.length === 0) {
      container.innerHTML =
        '<div class="col-span-full text-center text-slate-400 py-6">Aucune pièce justificative fournie.</div>';
      return;
    }
...
window.handleContractJobChange = function (jobId) {
  const job = window.folderContractJobs.find((j) => j.id == jobId);
  if (job) {
    document.getElementById("fcSalaireBase").value =
      job.salaireBrutReference || "";
  }
};
  ...
  try {
    ...
    if (structSelect) {
      structSelect.innerHTML =
        ...
        structures
          .map((s) => `<option value="${s.id}">${s.raisonSociale}</option>`)
          .join("");
    }
    ...
    if (funcSelect) {
      funcSelect.innerHTML =
        ...
        emplois
          .map((e) => `<option value="${e.id}">${e.libelle}</option>`)
          .join("");
    }
    ...
    const renewContainer = document.getElementById(
      "renewContractFormContainer",
    );
    const renewalsTbody = document.getElementById("folderRenewalsTableBody");

    if (!contrats || contrats.length === 0) {
      activeContainer.innerHTML =
        '<div class="text-center text-slate-400 py-4 font-semibold text-xs">Aucun contrat actif pour cet agent.</div>';
      createContainer.classList.remove("hidden");
      renewContainer.classList.add("hidden");
      renewalsTbody.innerHTML =
        '<tr><td colspan="5" class="p-2 text-center text-slate-400">Aucun historique.</td></tr>';
      window.currentActiveContractId = null;
      return;
    }
    ...
    if (!renewals || renewals.length === 0) {
      renewalsTbody.innerHTML =
        '<tr><td colspan="5" class="p-2 text-center text-slate-400">Aucun historique de renouvellement.</td></tr>';
    } else {
      renewalsTbody.innerHTML = renewals
        .map(
          (r) => `
                        <tr class="hover:bg-slate-50/50 transition-colors">
...
document
  .getElementById("folderCreateContractForm")
  ?.addEventListener("submit", async (e) => {
    e.preventDefault();
    const submitBtn = e.currentTarget.querySelector('button[type="submit"]');
    submitBtn.disabled = true;
    ...
    try {
      ...
      const selectedFunctionName =
        funcSelect && funcSelect.selectedIndex >= 0
...
document
  .getElementById("folderRenewContractForm")
  ?.addEventListener("submit", async (e) => {
    e.preventDefault();
    const submitBtn = e.currentTarget.querySelector('button[type="submit"]');
    submitBtn.disabled = true;
    ...
    try {
      ...
      document
        .getElementById("renewContractFormContainer")
...
async function loadFolderHardware() {
  ...
  try {
    ...
    if (!affectations || affectations.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="7" class="p-2 text-center text-slate-400">Aucun équipement assigné.</td></tr>';
    } else {
      tbody.innerHTML = affectations
        .map((a) => {
          const dateRemise = a.dateRemise
            ? new Date(a.dateRemise).toLocaleDateString("fr-FR")
            : "—";
          const dateRetour = a.dateRetour
            ? new Date(a.dateRetour).toLocaleDateString("fr-FR")
            : "";
          return `
                            <tr class="hover:bg-slate-50/50 transition-colors">
    ...
    const disponibles = (allMateriels || []).filter(
      (m) => m.statut === "DISPONIBLE",
    );

    if (disponibles.length === 0) {
      select.innerHTML =
        '<option value="">Aucun matériel disponible dans l\'inventaire...</option>';
    } else {
      select.innerHTML =
        '<option value="">Choisir un matériel à assigner...</option>' +
        disponibles
          .map(
            (m) =>
              `<option value="${m.id}">${m.categorie} - ${m.libelle} (S/N: ${m.numeroSerie || "—"})</option>`,
          )
          .join("");
    }
  } catch (err) {
...
async function loadClients() {
  try {
    ...
    siteZoneSelect.innerHTML =
      '<option value="">Associer à une Zone</option>' +
      zones.map((z) => `<option value="${z.id}">${z.nom}</option>`).join("");
    ...
    sitesTbody.innerHTML =
      sites
        ...
        .join("") ||
      '<tr><td colspan="4" class="p-3 text-center text-slate-400">Aucun site.</td></tr>';

    // Sites checklist in employeurs form
    const empSitesCheckboxList = document.getElementById(
      "empSitesCheckboxList",
    );
    empSitesCheckboxList.innerHTML =
      sites
        .map(
          (s) => `
    ...
    employeursTbody.innerHTML =
      employeurs
        ...
        .join("") ||
      '<tr><td colspan="4" class="p-3 text-center text-slate-400">Aucun compte employeur.</td></tr>';
  } catch (e) {
    console.error(e);
  }
}
...
document
  .getElementById("addStructureForm")
  ?.addEventListener("submit", async (e) => {
    e.preventDefault();
    try {
...
document
  .getElementById("addEmployeurForm")
  ?.addEventListener("submit", async (e) => {
    e.preventDefault();
    const checkedSites = Array.from(
      document.querySelectorAll('input[name="empSites"]:checked'),
    ).map((cb) => cb.value);
    try {
...
function filterAndShow(
  ...
  if (filtered.length === 0) {
    dropdown.innerHTML =
      '<div class="p-2 text-xs text-slate-400">Aucun résultat</div>';
  } else {
  ...
  try {
    ...
    setupAutocomplete(
      ...
      (selectedId) => {
        ...
        filteredEmployeurs = allEmployeurs.filter(
          (e) => e.structureId === selectedId,
        );
        ...
        setupAutocomplete(
          ...
          (selectedSiteId) => {
            ...
            if (site) {
              document.getElementById("affZoneInput").value =
                site.zoneNom || "—";
              document.getElementById("affVilleInput").value =
                site.adresse || "—";
            }
          },
        );
        ...
        affectations
          .map((a) => {
            const agentName =
              a.agentNom ||
              (a.agent
                ? `${a.agent.nom || ""} ${a.agent.prenom || ""}`.trim()
                : "") ||
              "Agent";

            // Site & Zone mapping
            const siteIdFromPoste =
              a.poste && a.poste.site ? a.poste.site.id : null;
            const foundSite = siteIdFromPoste
              ? allSites.find((s) => s.id === siteIdFromPoste)
              : null;
            const siteName = foundSite
              ? foundSite.nom
              : a.siteNom ||
                a.siteTravail ||
                (a.site ? a.site.nom : "Site non attribué");
            const posteName =
              a.posteLibelle ||
              (a.poste ? a.poste.libelle : a.agent ? a.agent.fonction : "") ||
              "Agent Terrain";

            const zoneName =
              foundSite && foundSite.zoneNom
                ? foundSite.zoneNom
                : a.zoneOperationnelle ||
                  a.zoneNom ||
                  (a.site &&
                    (a.site.zoneNom || (a.site.zone ? a.site.zone.nom : ""))) ||
                  (a.agent && a.agent.zoneNom) ||
                  "Non définie";
            const villeName =
              foundSite && foundSite.ville
                ? foundSite.ville
                : a.ville ||
                  (a.site && (a.site.ville || a.site.adresse)) ||
                  (a.agent && a.agent.ville) ||
                  "Non définie";

            // Supervisor mapping
            const supervisorId = a.employeurResponsable || a.superviseur;
            const foundEmp = supervisorId
              ? allEmployeurs.find(
                  (e) => e.id === supervisorId || e.email === supervisorId,
                )
              : null;
            let supervisorName = "—";
            if (foundEmp) {
              supervisorName =
                `${foundEmp.nom || ""} ${foundEmp.prenom || ""}`.trim() ||
                foundEmp.email;
            } else if (a.employeur) {
              supervisorName =
                `${a.employeur.nom || ""} ${a.employeur.prenom || ""}`.trim();
            } else if (supervisorId && !supervisorId.includes("-")) {
              supervisorName = supervisorId;
            }

            const hDebut =
              a.heureDebut || a.heureArriveeSite || a.heureArrivee || "08:00";
            const hFin =
              a.heureFin || a.heureDepartSite || a.heureDepart || "18:00";

            // Client Structure
            const clientName =
              a.structureCliente && a.structureCliente !== "—"
                ? a.structureCliente
                : a.client || "Client Inconnu";
          ...
          .join("") ||
        '<tr><td colspan="8" class="p-3 text-center text-slate-400">Aucune affectation.</td></tr>';
    }
  } catch (e) {
    console.error(e);
  }
...
async function loadPaieAndConfig() {
  try {
    ...
    if (config) {
      const configTauxCotisation = document.getElementById(
        "configTauxCotisation",
      );
      if (configTauxCotisation)
        configTauxCotisation.value = config.tauxCotisation || "0.00";
      const configSeuilAbsence = document.getElementById("configSeuilAbsence");
      if (configSeuilAbsence)
        configSeuilAbsence.value = config.seuilAbsenceLongueJours || "21";
      const configTauxRetenue = document.getElementById("configTauxRetenue");
      if (configTauxRetenue)
        configTauxRetenue.value = config.tauxRetenueReduite || "25.00";
    ...
    if (calcAffSelect) {
      const affectations = (await apiFetch("/admin/affectations")) || [];
      calcAffSelect.innerHTML =
        '<option value="">Choisir l\'Affectation</option>' +
        affectations
          .filter((a) => a.statut === "ACTIVE")
          .map(
            (a) =>
              `<option value="${a.id}">${a.agentNom} — ${a.posteLibelle}</option>`,
          )
          .join("");
    }
    ...
    if (factureStructSelect) {
      const structures = (await apiFetch("/organisation/structures")) || [];
      factureStructSelect.innerHTML =
        '<option value="">Choisir la Structure Cliente</option>' +
        structures
          .map((s) => `<option value="${s.id}">${s.raisonSociale}</option>`)
          .join("");
    }
    ...
    if (bulletinsTbody) {
      ...
      bulletinsTbody.innerHTML =
        bulletins
          ...
          .join("") ||
        '<tr><td colspan="6" class="p-3 text-center text-slate-400">Aucun bulletin de paie émis.</td></tr>';
    }
    ...
    if (facturesTbody) {
      ...
      facturesTbody.innerHTML =
        factures
          ...
          .join("") ||
        '<tr><td colspan="6" class="p-3 text-center text-slate-400">Aucune facture émise.</td></tr>';
    }
      ...
      try {
        ...
        primeRulesTbody.innerHTML =
          primeRules
            ...
            .join("") ||
          '<tr><td colspan="3" class="p-1 text-center text-slate-400">Aucune règle.</td></tr>';
      } catch (e) {
...
document
  .getElementById("updateConfigForm")
  ?.addEventListener("submit", async (e) => {
    e.preventDefault();
    try {
      await apiFetch("/admin/entreprise/config", {
        ...
        body: JSON.stringify({
          ...
          seuilAbsenceLongueJours: parseInt(
            document.getElementById("configSeuilAbsence").value,
            10,
          ),
          tauxRetenueReduite:
            document.getElementById("configTauxRetenue").value,
        }),
      });
...
document
  .getElementById("calculBulletinForm")
  ?.addEventListener("submit", async (e) => {
    e.preventDefault();
    try {
      await apiFetch("/paie/bulletins/generer", {
        ...
        body: JSON.stringify({
          ...
          joursPrevus: parseInt(
            document.getElementById("calcJoursPrevus").value,
            10,
          ),
          joursValides: parseInt(
            document.getElementById("calcJoursValides").value,
            10,
          ),
          joursAbsJustCourte: parseInt(
            document.getElementById("calcAbsJustCourte").value || "0",
            10,
          ),
          joursAbsJustLongue: parseInt(
            document.getElementById("calcAbsJustLongue").value || "0",
            10,
          ),
          joursAbsNonJust: parseInt(
            document.getElementById("calcAbsNonJust").value || "0",
            10,
          ),
          joursCongePaye: parseInt(
            document.getElementById("calcConges").value || "0",
            10,
          ),
        }),
      });
...
document
  .getElementById("genererFactureForm")
  ?.addEventListener("submit", async (e) => {
    e.preventDefault();
    try {
      await apiFetch("/paie/factures/generer", {
        ...
        body: JSON.stringify({
          ...
          rapportUrl:
            document.getElementById("factureRapportUrl").value || null,
        }),
      });
      closeFactureModal();
...
document
  .getElementById("addPrimeRuleForm")
  ?.addEventListener("submit", async (e) => {
    e.preventDefault();
    try {
      await apiFetch("/primes/rendement/regles", {
        ...
        body: JSON.stringify({
          ...
          montantParPoint: parseFloat(
            document.getElementById("primeMontantParPoint").value,
          ),
          seuilMinimum: parseInt(
            document.getElementById("primeSeuilMinimum").value,
            10,
          ),
          statut: "ACTIF",
        }),
...
document
  .getElementById("simulatePrimeForm")
  ?.addEventListener("submit", async (e) => {
    e.preventDefault();
    try {
      const res = await apiFetch("/primes/rendement/simuler", {
        ...
        body: JSON.stringify({
          ...
          montantParPoint:
            document.getElementById("simMontantParPoint").value || "0",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("affDateDebut").value = new Date()
    .toISOString()
    .split("T")[0];
  ...
  document
    .getElementById("addSanctionForm")
    ?.addEventListener("submit", async (e) => {
      e.preventDefault();
      const btn = e.target.querySelector('button[type="submit"]');
      const origText = btn.textContent;
      ...
      try {
        ...
        const dateFin =
          document.getElementById("sanctionDateFin").value || null;
        const motif = document.getElementById("sanctionMotif").value;
        const decisionUrl =
          document.getElementById("sanctionDecisionUrl").value || null;
        ...
        console.error(err);
        alert(
          "Erreur lors de l'enregistrement de la sanction : " + err.message,
        );
      } finally {
        btn.disabled = false;
        btn.textContent = origText;
  ...
  document
    .getElementById("addEvaluationForm")
    ?.addEventListener("submit", async (e) => {
      e.preventDefault();
      const btn = e.target.querySelector('button[type="submit"]');
      const origText = btn.textContent;
      ...
      try {
        ...
        const ponctualite = parseInt(
          document.getElementById("evalPonctualite").value,
        );
        const discipline = parseInt(
          document.getElementById("evalDiscipline").value,
        );
        const qualite = parseInt(document.getElementById("evalQualite").value);
        const productivite = parseInt(
          document.getElementById("evalProductivite").value,
        );
        const espritEquipe = parseInt(
          document.getElementById("evalEspritEquipe").value,
        );
        const respectProcedures = parseInt(
          document.getElementById("evalRespectProcedures").value,
        );
        const satisfactionClient = parseInt(
          document.getElementById("evalSatisfactionClient").value,
        );
        const communication = parseInt(
          document.getElementById("evalCommunication").value,
        );
        ...
        console.error(err);
        alert(
          "Erreur lors de l'enregistrement de l'évaluation : " + err.message,
        );
      } finally {
        btn.disabled = false;
        btn.textContent = origText;
  ...
  try {
    ...
    safeRenderTbody(
      ...
      (contrats || [])
        ...
        .join("") ||
        '<tr><td colspan="8" class="p-3 text-center text-slate-400">Aucun contrat.</td></tr>',
    );
  } catch (e) {
    console.error(e);
    safeRenderTbody(
      document.getElementById("contratsTableBody"),
      '<tr><td colspan="8" class="p-3 text-center text-red-500">Erreur chargement contrats</td></tr>',
    );
  }
}
...
window.checkContractExpirations = async function () {
  try {
    ...
    if (list.length > 0) {
      ...
      if (agentNameDisplay)
        agentNameDisplay.textContent =
          "Renouvellement " + (c.agentNom || "Agent");
      if (dateDisplay) dateDisplay.textContent = "Date Limite : " + c.dateFin;
      if (btnDisplay)
        btnDisplay.onclick = function () {
          openAgentFolder(c.agentId || c.agent?.id);
          setTimeout(() => switchFolderTab("contract"), 400);
        };
    } else {
      const agentNameDisplay = document.getElementById("reminderAgentName");
      const dateDisplay = document.getElementById("reminderDate");
      if (agentNameDisplay)
        agentNameDisplay.textContent = "Aucun renouvellement";
    ...
    console.error(
      "Erreur lors de la vérification des expirations de contrats:",
      e,
    );
  }
};
...
function renderPointageRows(data) {
  ...
  if (!data || data.length === 0) {
    safeRenderTbody(
      tbody,
      '<tr><td colspan="9" class="p-4 text-center text-slate-400">Aucun pointage pour cette date.</td></tr>',
    );
    return;
  }
  safeRenderTbody(
...
function updatePointageStats(data) {
  ...
  const durees = data
    .filter((p) => p.dureeMinutes > 0)
    .map((p) => p.dureeMinutes);
  const avgDuree = durees.length
  ...
  document.getElementById("pointageCount").textContent =
    `${total} enregistrement${total > 1 ? "s" : ""}`;
}

function filterPointageTable() {
  const agentQ = (
    document.getElementById("pointageAgentFilter")?.value || ""
  ).toLowerCase();
  const modeQ = document.getElementById("pointageModeFilter")?.value || "";
  const structQ =
    document.getElementById("pointageStructureFilter")?.value || "";

  const filtered = _allPointages.filter((p) => {
    const matchAgent =
      !agentQ || (p.agentNom || "").toLowerCase().includes(agentQ);
  ...
  try {
    ...
    if (label)
      label.textContent = dateVal
        ? `Pointages du ${dateVal}`
        : "Pointages du jour";

    const url = dateVal ? `/pointages?date=${dateVal}` : "/pointages/today";
    safeRenderTbody(
      document.getElementById("pointagesTableBody"),
    ...
    if (structFilter) {
      const structs = [
        ...new Set(
          _allPointages
            .map((p) => p.structureCliente)
            .filter((s) => s && s !== "—"),
        ),
      ];
      const current = structFilter.value;
      structFilter.innerHTML =
...
async function loadPointageDates() {
  try {
    ...
    if (!dates.length) {
      safeRenderTbody(
        tbody,
        '<tr><td colspan="3" class="p-4 text-center text-slate-400">Aucun historique disponible.</td></tr>',
      );
      return;
    }
    safeRenderTbody(
    ...
    safeRenderTbody(
      document.getElementById("pointageDatesBody"),
...
function viewDatePointages(date) {
  ...
  document
    .getElementById("pointagesTableBody")
    ?.closest(".glass")
...
async function loadPresences() {
  try {
    ...
    if (!mois) {
      safeRenderTbody(
        document.getElementById("presencesTableBody"),
        '<tr><td colspan="5" class="p-3 text-center text-slate-400">Sélectionnez un mois.</td></tr>',
      );
      return;
    }
    ...
    safeRenderTbody(
      document.getElementById("presencesTableBody"),
      (rowsHtml ||
        '<tr><td colspan="8" class="p-3 text-center text-slate-400">Aucune présence.</td></tr>') +
        totalRowHtml,
    );
  } catch (e) {
    console.error(e);
    safeRenderTbody(
      document.getElementById("presencesTableBody"),
      '<tr><td colspan="5" class="p-3 text-center text-red-500">Erreur de chargement.</td></tr>',
    );
  }
}
...
window.exportPresences = async function (format) {
  try {
    ...
    const response = await fetch(
      `/api/presences/export?format=${encodeURIComponent(format)}&mois=${encodeURIComponent(mois)}`,
      {
        headers,
      },
    );
    ...
    if (!response.ok) {
      throw new Error(
        "Erreur lors de l'exportation des présences: " + response.statusText,
      );
    }
...
async function loadMateriel() {
  try {
    ...
    try {
      ...
      if (assignZoneSelect) {
        assignZoneSelect.innerHTML =
          '<option value="">Sélectionner une zone...</option>' +
          zones
            .map(
              (z) =>
                `<option value="${z.id}">${z.nom} (${z.villes ? (Array.isArray(z.villes) ? z.villes.join(", ") : z.villes) : z.perimetre || ""})</option>`,
            )
            .join("");
      }
      ...
      if (assignCoordSelect) {
        assignCoordSelect.innerHTML =
          '<option value="">Aucun (Assigner à la Zone uniquement)</option>' +
          coords
            .map(
              (c) =>
                `<option value="${c.id}">${c.nom} ${c.prenom} (${c.zoneNom || "Sans zone"})</option>`,
            )
            .join("");
      }
    } catch (errPop) {
    ...
    if (displayElem)
      displayElem.textContent = `${materiels.length} équipement(s)`;

    // Populate disponible materiel select for zone assignment
    const assignZoneMaterielSelect = document.getElementById(
      "assignZoneMaterielSelect",
    );
    if (assignZoneMaterielSelect) {
      const disponibles = materiels.filter(
        (m) => (m.statut || "").toUpperCase() === "DISPONIBLE",
      );
      if (disponibles.length === 0) {
        assignZoneMaterielSelect.innerHTML =
        ...
        assignZoneMaterielSelect.innerHTML =
          ...
          disponibles
            .map(
              (m) =>
                `<option value="${m.id}">${m.libelle || m.nom} - S/N: ${m.numeroSerie || m.serialNumber || "—"}</option>`,
            )
            .join("");
      }
    }
    ...
    safeRenderTbody(
      document.getElementById("materielTableBody"),
      materiels
        .map((m) => {
          let statusBadge = "";
          ...
          const val =
            m.valeurAchat != null
              ? `${Number(m.valeurAchat).toFixed(2)} FCFA`
              : "—";
          ...
          const zoneText =
            m.zone && m.zone.nom ? m.zone.nom : m.zoneNom || null;
          const coordText = m.coordonnateur
            ? `${m.coordonnateur.nom || ""} ${m.coordonnateur.prenom || ""}`.trim()
            : m.coordonnateurNom || null;
        ...
        .join("") ||
        '<tr><td colspan="6" class="p-3 text-center text-slate-400">Aucun matériel dans l\'inventaire.</td></tr>',
    );

    const paramMaterielTbody = document.getElementById(
      "paramMaterielTableBody",
    );
    if (paramMaterielTbody) {
      paramMaterielTbody.innerHTML =
        materiels
          .map((m) => {
            let statusBadge = "";
            const st = (m.statut || "").toUpperCase();
          ...
          .join("") ||
        '<tr><td colspan="5" class="p-3 text-center text-slate-400">Aucun matériel.</td></tr>';
    }

    // Chargement des demandes
    const demandes = (await apiFetch("/materiels/demandes")) || [];
    safeRenderTbody(
      document.getElementById("adminMaterielRequestsTable"),
      demandes
        .map((d) => {
          let actionButtons = "";
          ...
          if (d.statut === "APPROUVE") {
            statusBadge =
              '<span class="badge bg-green-100 text-green-700 font-bold">Approuvée</span>';
          } else if (d.statut === "REFUSE") {
            statusBadge =
              '<span class="badge bg-red-100 text-red-700 font-bold">Refusée</span>';
          } else {
            statusBadge =
              '<span class="badge bg-amber-100 text-amber-700 font-bold">En attente</span>';
          }

          const val =
            d.valeurAchat != null
              ? `${Number(d.valeurAchat).toFixed(2)} FCFA`
              : "—";
          ...
          const coordName =
            d.coordonnateurNom ||
            (d.coordonnateur
              ? `${d.coordonnateur.nom} ${d.coordonnateur.prenom}`
              : "—");
          const zoneName =
            d.zoneNom ||
            d.zone ||
            (d.coordonnateur ? d.coordonnateur.zoneNom : null);
        ...
        .join("") ||
        '<tr><td colspan="8" class="p-3 text-center text-slate-400">Aucune demande de matériel reçue.</td></tr>',
    );
    // Chargement de l'historique d'affectation
    try {
      const historique = (await apiFetch("/materiels/historique")) || [];
      safeRenderTbody(
        document.getElementById("adminMaterielHistoryTable"),
        historique
          .map((h) => {
            let statBadge = "";
            if (h.dateRetour) {
              statBadge =
                '<span class="badge bg-slate-100 text-slate-700 font-semibold">Rendu</span>';
            } else {
              statBadge =
                '<span class="badge bg-sky-100 text-sky-700 font-bold">En cours</span>';
            }
            return `
                                <tr class="hover:bg-slate-50/50 transition-colors">
          ...
          .join("") ||
          '<tr><td colspan="8" class="p-3 text-center text-slate-400">Aucun historique d\'affectation.</td></tr>',
      );
    } catch (e) {
    ...
    safeRenderTbody(
      document.getElementById("materielTableBody"),
      '<tr><td colspan="6" class="p-3 text-center text-red-500">Erreur chargement matériel</td></tr>',
    );
  }
}
...
window.assignMaterielToZone = async function () {
  ...
  const coordonnateurId =
    document.getElementById("assignCoordSelect")?.value || null;

  if (!materielId || !zoneId) {
    alert(
      "Veuillez sélectionner le matériel à assigner et la zone de destination.",
    );
    return;
  }
  ...
  try {
    ...
    try {
      ...
      alert(
        "Erreur lors de l'assignation du matériel : " + errFallback.message,
      );
    }
  }
};
...
window.paramCreateEmploi = async function () {
  try {
    ...
    const description = document
      .getElementById("paramEmploiDescription")
      .value.trim();
    const categorie = document
      .getElementById("paramEmploiCategorie")
      .value.trim();
    const competences = document
      .getElementById("paramEmploiCompetences")
      .value.trim();
...
window.paramCreateMateriel = async function () {
  try {
    const libelle = document
      .getElementById("paramMaterielLibelle")
      .value.trim();
    const categorie = document.getElementById("paramMaterielCategorie").value;
    const numeroSerie = document
      .getElementById("paramMaterielNumeroSerie")
      .value.trim();
    const imei = document.getElementById("paramMaterielImei").value.trim();
    const valeurAchatStr = document.getElementById(
      "paramMaterielValeurAchat",
    ).value;
...
window.adminCreateMateriel = async function () {
  try {
    ...
    const numeroSerie = document
      .getElementById("addMaterielNumeroSerie")
      ?.value?.trim();
    const valeurAchatStr = document.getElementById(
      "addMaterielValeurAchat",
    )?.value;
    ...
    if (document.getElementById("addMaterielNumeroSerie"))
      document.getElementById("addMaterielNumeroSerie").value = "";
    if (document.getElementById("addMaterielValeurAchat"))
      document.getElementById("addMaterielValeurAchat").value = "";
...
window.declarerIncidentMateriel = async function (id, libelle) {
  ...
  if (
    choixStatut.trim() === "2" ||
    choixStatut.toUpperCase().includes("INUTILISABLE")
  ) {
    statut = "INUTILISABLE";
  } else if (
    choixStatut.trim() === "3" ||
    choixStatut.toUpperCase().includes("PERDU")
  ) {
    statut = "PERDU";
  } else if (
    choixStatut.trim() === "1" ||
    choixStatut.toUpperCase().includes("DEFECTUEUX") ||
    choixStatut.toUpperCase().includes("PANNE")
  ) {
    statut = "DEFECTUEUX";
  } else {
  ...
  const details = prompt(
    `Précisez les détails ou le motif de l'incident (ex: Chute lors de la mission, bouton radio cassé...) :`,
    "",
  );
  ...
  try {
    ...
    alert(
      `Incident enregistré (${statut}) ! L'état du matériel a été mis à jour.`,
    );
    loadMateriel();
  } catch (e) {
    console.error(e);
...
async function loadConges() {
  try {
    ...
    safeRenderTbody(
      document.getElementById("congesTableBody"),
      window.congesList
        .map((c) => {
          const actionButtons = `
        ...
        .join("") ||
        '<tr><td colspan="9" class="p-3 text-center text-slate-400">Aucune demande de congé ou absence en cours.</td></tr>',
    );
  } catch (e) {
    console.error(e);
    safeRenderTbody(
      document.getElementById("congesTableBody"),
      '<tr><td colspan="8" class="p-3 text-center text-red-500">Erreur chargement congés</td></tr>',
    );
  }
}
...
window.viewCongeDetails = function (id) {
  ...
  document.getElementById("congeModalAgentName").textContent =
    "Agent : " + (c.agent ? `${c.agent.nom} ${c.agent.prenom}` : "—");
  ...
  document.getElementById("congeModalMotif").textContent =
    c.motif || "Aucun motif renseigné.";
  document.getElementById("congeModalStructure").textContent =
    c.structureCliente || "—";
  ...
  if (isExpired || c.statut === "VALIDEE")
    ...
    badge = `<span class="px-2 py-1 bg-amber-100 text-amber-700 font-bold rounded text-[10px]">${c.statut.replace("EN_ATTENTE_", "Attente ")}</span>`;
  ...
  if (
    c.statut === "EN_ATTENTE_DIRECTION" ||
    c.statut === "EN_ATTENTE_RH" ||
    c.statut === "EN_ATTENTE_SUPERVISEUR"
  ) {
...
async function loadDisciplinaire() {
  try {
    ...
    if (sanctionAgentSelect) {
      sanctionAgentSelect.innerHTML = agents
        .map(
          (a) =>
    ...
    console.error(e);
    safeRenderTbody(
      document.getElementById("sanctionsTableBody"),
      '<tr><td colspan="8" class="p-3 text-center text-red-500">Erreur chargement sanctions</td></tr>',
    );
  }
}
  ...
  safeRenderTbody(
    ...
    list
      .map((s) => {
        ...
        const effectifStatut = sanctionExpired
          ? "TERMINE"
          : s.statut || "EN_COURS";
        const badgeColor =
          effectifStatut === "VALIDE" || effectifStatut === "TERMINE"
      ...
      .join("") ||
      '<tr><td colspan="8" class="p-3 text-center text-slate-400">Aucune sanction.</td></tr>',
  );
}

window.filterSanctionsTable = function () {
  const query =
    document.getElementById("searchSanctions")?.value?.toLowerCase() || "";
  if (!window.allSanctionsList) return;
  const filtered = window.allSanctionsList.filter((s) => {
    const name = s.agent
      ? `${s.agent.nom || ""} ${s.agent.prenom || ""}`.toLowerCase()
      : (s.agentNom || "").toLowerCase();
...
async function loadEvaluations() {
  try {
    ...
    if (evalAgentSelect) {
      evalAgentSelect.innerHTML = agents
        .map(
          (a) =>
            `<option value="${a.id}">${a.nom || ""} ${a.prenom || ""} (${a.matricule || "Sans matricule"})</option>`,
        )
    ...
    console.error(e);
    safeRenderTbody(
      document.getElementById("evaluationsTableBody"),
    ...
    list
      .map((ev) => {
        ...
        const structureEv =
          ...
          ev.clientFinal ||
          (ev.agent &&
            (ev.agent.structureCliente ||
              ev.agent.clientFinal ||
              ev.agent.structure)) ||
          "—";
        const evaluateurEv =
          ev.employeurEvaluateur || ev.evaluateurNom || ev.evaluateur || "—";
        return `
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <td class="p-3 font-bold text-slate-800">${agentNom}</td>
      ...
      .join("") ||
      '<tr><td colspan="7" class="p-3 text-center text-slate-400">Aucune évaluation.</td></tr>',
  );
}

window.filterEvaluationsTable = function () {
  const query =
    document.getElementById("searchEvaluations")?.value?.toLowerCase() || "";
...
async function loadRapports(type) {
  try {
    if (
      !type &&
      document.getElementById("rapportAgentSelect") &&
      document.getElementById("rapportAgentSelect").options.length <= 1
    ) {
      if (typeof window.switchRapportMode === "function")
        window.switchRapportMode("agent");
    }
    ...
    const reportType =
      type || document.getElementById("rapportsTypeSelect")?.value || "global";
    const payload = await apiFetch(`/rapports/${reportType}?mois=${mois}`);

    if (document.getElementById("rapportsPreview")) {
      document.getElementById("rapportsPreview").textContent = JSON.stringify(
        payload,
        null,
        2,
      );
    ...
    console.error(e);
    if (document.getElementById("rapportsPreview")) {
      document.getElementById("rapportsPreview").textContent =
        "Erreur chargement rapports: " + (e.message || e);
    }
...
window.renderVisualRapportPreview = function (payload) {
  ...
  const dateGen =
    payload.dateGeneration || new Date().toISOString().split("T")[0];
                            ...
                            <table class="min-w-full text-left text-xs whitespace-nowrap">
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    ${(() => {
                                      ...
                                      return (
                                        rows ||
                                        '<tr><td colspan="8" class="p-3 text-center text-slate-400">Aucun pointage enregistré sur la période.</td></tr>'
                                      );
                                    })()}
                                </tbody>
                            </table>
                                    ...
                                    ${
                                      list.length
                                        ...
                                        `;
                                            ...
                                            .join("")
                                        : '<tr><td colspan="5" class="p-3 text-center text-slate-400">Aucune demande de congé enregistrée sur la période.</td></tr>'
                                    }
                                </tbody>
                            </table>
                        </div>
                                    ...
                                    ${
                                      list.length
                                        ...
                                        `;
                                            ...
                                            .join("")
                                        : '<tr><td colspan="5" class="p-3 text-center text-slate-400">Aucun équipement enregistré dans l\'inventaire.</td></tr>'
                                    }
                                </tbody>
                            </table>
                        </div>
                            ...
                            <table class="min-w-full text-left text-xs whitespace-nowrap">
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    ...
                                    `,
                                            ...
                                            .join("")
                                        : '<tr><td colspan="5" class="p-3 text-center text-slate-400">Aucune sanction enregistrée sur la période.</td></tr>'
                                    }
                                </tbody>
                            </table>
                        </div>
...
window.exportRapport = async function (type, format) {
  try {
    ...
    const response = await fetch(
      `/api/rapports/${reportType}/export?format=${encodeURIComponent(format)}&mois=${encodeURIComponent(mois)}`,
      {
        headers,
      },
    );
    ...
    if (!response.ok) {
      throw new Error(
        "Erreur lors de l'exportation du rapport: " + response.statusText,
      );
    }
    ...
    safeRenderTbody(
      ...
      (logs || [])
        .map((l) => {
          ...
          const user =
            l.utilisateurEmail || l.utilisateur || l.username || "Système";
          const act = l.action || l.type || "—";
          const mod = l.module
            ? `<span class="px-2 py-0.5 rounded text-[10px] font-bold bg-slate-100 text-slate-700 mr-1">${l.module}</span>`
            : "";
          const det = l.details
            ? `<div class="text-[11px] text-slate-400 mt-0.5">${l.details}</div>`
            : "";
          return `
                            <tr class="hover:bg-slate-50/50 transition-colors">
        ...
        .join("") ||
        '<tr><td colspan="3" class="p-3 text-center text-slate-400">Aucun log.</td></tr>',
    );
  } catch (e) {
    console.error(e);
    safeRenderTbody(
      document.getElementById("auditTableBody"),
      '<tr><td colspan="3" class="p-3 text-center text-red-500">Erreur chargement audit</td></tr>',
    );
  }
}
...
window.clearAuditLog = async function () {
  if (!confirm("Etes-vous sur de vouloir vider tout le journal d'audit ?"))
    return;
  try {
    await apiFetch("/audit", { method: "DELETE" });
  } catch (e) {
    console.log("DELETE audit not implemented, clearing display only");
  }
  safeRenderTbody(
    document.getElementById("auditTableBody"),
    '<tr><td colspan="3" class="p-3 text-center text-slate-400">Journal vide.</td></tr>',
  );
...
document
  .getElementById("congeJustifFile")
  ?.addEventListener("change", async (e) => {
    ...
    try {
      ...
      document.getElementById("congeJustifUrl").value =
      ...
      statusEl.className = "text-[10px] text-red-500 mt-1 block";
    }
  });
...
window.loadRapportAgent = async function () {
  var agentId =
    document.getElementById("rapportAgentSelect") &&
    document.getElementById("rapportAgentSelect").value;
  var mois =
    document.getElementById("rapportAgentMois") &&
    document.getElementById("rapportAgentMois").value;
  var type =
    (document.getElementById("rapportAgentType") &&
      document.getElementById("rapportAgentType").value) ||
    "global";
  var container = document.getElementById("rapportAgentVisual");
  if (!agentId) {
    showToast && showToast("Selectionnez un agent.", "error");
    return;
  }
  if (container)
    container.innerHTML =
      '<div class="text-center text-slate-400 text-xs p-8"><i class="fa-solid fa-spinner fa-spin text-2xl mb-2 block"></i>Chargement...</div>';
  try {
    ...
    if (container) {
      container.innerHTML =
        ...
        '<div><h4 class="text-lg font-extrabold text-slate-900">' +
        nomAgent +
        "</h4>" +
        ...
        '<div class="bg-blue-50 rounded-xl p-4 text-center"><div class="text-2xl font-black text-blue-700">' +
        pt.length +
        '</div><div class="text-[10px] font-bold text-blue-500 mt-1 uppercase tracking-wide">Presences</div></div>' +
        '<div class="bg-amber-50 rounded-xl p-4 text-center"><div class="text-2xl font-black text-amber-700">' +
        cg.filter(function (x) {
          return x.statut === "VALIDEE";
        }).length +
        '</div><div class="text-[10px] font-bold text-amber-500 mt-1 uppercase tracking-wide">Conges</div></div>' +
        '<div class="bg-red-50 rounded-xl p-4 text-center"><div class="text-2xl font-black text-red-700">' +
        sc.length +
        '</div><div class="text-[10px] font-bold text-red-500 mt-1 uppercase tracking-wide">Sanctions</div></div>' +
        '<div class="bg-indigo-50 rounded-xl p-4 text-center"><div class="text-2xl font-black text-indigo-700">' +
        ms.length +
        '</div><div class="text-[10px] font-bold text-indigo-500 mt-1 uppercase tracking-wide">Missions</div></div>' +
        "</div>" +
        (cg.length > 0
          ? '<div><h5 class="text-xs font-bold text-slate-600 uppercase tracking-wide mb-2">Derniers conges / absences</h5><div class="space-y-1.5">' +
            cg
              .slice(0, 5)
              .map(function (x) {
                return (
                  '<div class="flex justify-between items-center bg-slate-50 rounded-lg px-3 py-2 text-xs"><span class="font-semibold text-slate-700">' +
                  (x.type || "-") +
                  '</span><span class="text-slate-400">' +
                  (x.dateDebut || "") +
                  " - " +
                  (x.dateFin || "") +
                  '</span><span class="px-2 py-0.5 rounded text-[10px] font-bold ' +
                  (x.statut === "VALIDEE"
                    ? "bg-green-100 text-green-700"
                    : "bg-amber-100 text-amber-700") +
                  '">' +
          ...
          : "") +
        (sc.length > 0
          ? '<div><h5 class="text-xs font-bold text-slate-600 uppercase tracking-wide mb-2">Sanctions disciplinaires</h5><div class="space-y-1.5">' +
            sc
              .slice(0, 5)
              .map(function (x) {
                return (
                  '<div class="flex justify-between items-center bg-red-50 rounded-lg px-3 py-2 text-xs"><span class="font-semibold text-red-700">' +
                  (x.type || "-") +
                  '</span><span class="text-slate-500 truncate max-w-xs">' +
                  (x.motif || "-") +
                  "</span></div>"
                );
              })
    ...
    if (container)
      container.innerHTML =
        '<div class="p-4 bg-rose-50 text-rose-700 border border-rose-200 rounded-xl text-xs font-bold">Erreur : ' +
        err.message +
        "</div>";
  }
};
...
window.exportRapportAgent = async function (fmt) {
  ...
  try {
    ...
    a.download =
      "rapport_agent_" + agentId + "_" + (mois || "complet") + ".pdf";
    a.click();
...
window.zoomQr = function (fullName, qrData, zone) {
  ...
  QRCode.toCanvas(
    canvas,
    qrData,
    { width: 220, margin: 2, color: { dark: "#1e1b4b", light: "#ffffff" } },
    (err) => {
      if (err) console.error(err);
    },
  );

  document.getElementById("adminDownloadPdfBtn").onclick = () =>
    generateAdminBadgePdf(fullName, qrData, zone || "");
  document.getElementById("adminPrintQrBtn").onclick = () =>
    printAdminBadge(fullName, qrData, zone || "");
};
...
window.generateAdminBadgePdf = function (fullName, qrData, zone) {
  QRCode.toDataURL(
    ...
    function (err, qrDataUrl) {
      ...
      doc.setFont("helvetica", "normal");
      doc.text("Agent Terrain" + (zone ? " — " + zone : ""), W / 2, 119, {
        align: "center",
      });
      ...
      doc.setFontSize(9);
      doc.setTextColor(100, 116, 139);
      doc.text("Scannez ce code lors de chaque pointage", W / 2, 231, {
        align: "center",
      });
      ...
      doc.setFontSize(8);
      doc.text(
        "SimpleTaff — Plateforme de Gestion du Personnel",
        W / 2,
        H - 5,
        { align: "center" },
      );
...
window.printAdminBadge = function (fullName, qrData, zone) {
  QRCode.toDataURL(
    ...
    function (err, qrDataUrl) {
      ...
      w.document
        .write(`<!DOCTYPE html><html><head><title>Badge — ${fullName}</title>
...
window.fallbackAvatar = function (imgEl) {
  imgEl.onerror = null;
  const initial = (imgEl.getAttribute("data-initial") || "A")
    .charAt(0)
    .toUpperCase();
  const span = document.createElement("span");
  span.className =
    "inline-flex w-8 h-8 rounded-full bg-slate-200 items-center justify-center text-slate-500 font-bold text-xs";
...
window.loadNotifications = async function () {
  try {
    ...
    if (unread.length > lastUnreadCount) {
      if (window.showToast)
        window.showToast("Vous avez de nouvelles notifications", "info");
    }
    ...
    container.innerHTML =
      validNotifications
        ...
        .join("") ||
      '<div class="p-4 text-center text-slate-400">Aucune notification.</div>';
  } catch (e) {
    console.error(e);
  }
};
...
if (searchInput) {
  searchInput.addEventListener("input", function (e) {
    ...
    if (
      typeof window.entreprises !== "undefined" &&
      window.entreprises.length > 0
    ) {
    ...
    if (results.length > 0) {
      ...
      searchContainer.innerHTML =
        '<div class="p-4 text-center text-slate-400 text-sm">Aucun résultat trouvé</div>';
      searchDropdown.classList.remove("hidden");
    }
  });
