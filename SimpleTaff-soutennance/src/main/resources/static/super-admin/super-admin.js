...
window.loadNotifications = async function () {
  try {
    ...
    const validNotifications = allNotifs.filter(
      (n) => n.type === "SUPER_ADMIN",
    );
      ...
      if (validNotifications.length === 0) {
        ...
        container.innerHTML = validNotifications
          ...
          .map((n) => {
            ...
            const dateStr = !isNaN(dateMatch)
              ? dateMatch.toLocaleDateString() +
                " " +
                dateMatch.toLocaleTimeString([], {
                  hour: "2-digit",
                  minute: "2-digit",
                })
              : "";
...
window.loadEntreprises = async function () {
  ...
  try {
    ...
    const actives = data.filter(
      (e) => e.statut === "ACTIF" || e.statut === "ACTIVE",
    ).length;
    const pending = data.filter(
      (e) => e.statut === "INACTIF" || e.statut === "SUSPENDUE",
    ).length;
...
window.deleteEntreprise = async function (id, nom) {
  if (
    !confirm(
      `ATTENTION : Voulez-vous vraiment supprimer définitivement l'entreprise "${nom}" ? Cette action est irréversible et effacera toutes les données (agents, utilisateurs, pointages, factures...) de cette entreprise.`,
    )
  )
    return;
  try {
...
async function loadOverview() {
  try {
    ...
    if (res) {
      if (document.getElementById("statActives")) {
        document.getElementById("statActives").textContent =
          res.totalClients || 0;
      }
      if (document.getElementById("statPending")) {
        document.getElementById("statPending").textContent =
          res.abonnementsActifs || 0;
      }
      if (document.getElementById("statTotal")) {
        document.getElementById("statTotal").textContent =
          res.coordonnateurs || 0;
      }

      if (
        document.getElementById("revenuSaasVal") &&
        res.chartData &&
        ...
        if (container && labelsContainer) {
          ...
          container.innerHTML = res.chartData
            .map((d, index) => {
          ...
          labelsContainer.innerHTML = res.chartData
            .map((d, index) => {
              const isCurrent = index === res.chartData.length - 1;
              const colorClass = isCurrent
                ? "text-[#12312E] font-extrabold"
                : "";
      ...
      if (
        document.getElementById("abonnementExpireNom") &&
        document.getElementById("abonnementExpireDate")
      ) {
        if (res.abonnementExpire && res.abonnementExpire.nom) {
          document.getElementById("abonnementExpireNom").textContent =
            res.abonnementExpire.nom;
          ...
          document.getElementById("abonnementExpireDate").textContent =
            `Expiré le : ${d.toLocaleDateString()}`;
        } else {
          document.getElementById("abonnementExpireNom").textContent =
      ...
      if (adminsTbody && res.nouveauxAdmins) {
        if (res.nouveauxAdmins.length === 0) {
          adminsTbody.innerHTML =
            '<tr><td class="py-2 text-slate-500 text-center">Aucun administrateur récent</td></tr>';
        } else {
          adminsTbody.innerHTML = res.nouveauxAdmins
            .map(
              (a) => `
                                <tr>
    ...
    if (window.admins) {
      window.admins.forEach((a) => {
        ...
        if (
          ...
          (a.entreprise_nom || "").toLowerCase().includes(q)
        ) {
          results.push({
            type: "Administrateur",
            icon: "👤",
