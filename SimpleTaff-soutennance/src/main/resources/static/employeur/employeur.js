...
async function loadPersonnelQr() {
  ...
  try {
    ...
    for (const agent of agents) {
      ...
      card.className =
        "glass rounded-2xl p-4 flex flex-col items-center gap-3 text-center shadow-sm hover:shadow-md transition-shadow";
      ...
      const initiales = (agent.agentNom || "AG")
        .split(" ")
        .map((p) => p[0])
        .join("")
        .toUpperCase()
...
function startScanning(typePointage = null) {
  ...
  html5QrCode
    ...
    .catch((err) => {
      ...
      alert(
        "Impossible de démarrer la caméra. Assurez-vous d'avoir donné les permissions.",
      );
      stopScanning();
    });
}
...
function setPointageMode(mode) {
  ...
  if (mode === "QR_CODE") {
    document.getElementById("labelAgentCard").textContent =
      "Identifiant (QR / NFC / Biometrie)";
  } else if (mode === "NFC") {
    document.getElementById("labelAgentCard").textContent =
      "Identifiant NFC (tag)";
  } else if (mode === "BIOMETRIE") {
    document.getElementById("labelAgentCard").textContent =
      "Identifiant Biométrie (ID lecteur)";
...
async function enregistrerPointage(typePointage) {
  ...
  try {
    ...
    const payload = {
      ...
      qrCode:
        activePointageMode === "QR_CODE" || activePointageMode === "PHOTO_GPS"
          ? cardId
          : undefined,
    ...
    pendingPointageType = null;
    msg.textContent =
      err.message || "Erreur lors de la validation du pointage.";
    msg.classList.add("bg-red-50", "text-red-700");
    msg.classList.remove("hidden");
  }
}
  ...
  try {
    ...
    try {
      ...
      if (statsRes) {
        ...
        document.getElementById("statOverviewPointagesDay").textContent =
          statsRes.postesActifs || 0;
        ...
        if (iSatisfaction) {
          ...
          const satisfactionCircle =
            document.getElementById("satisfactionCircle");
    ...
    if (!agents || agents.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="4" class="px-6 py-6 text-center text-slate-400">Aucun agent assigné sur votre site.</td></tr>';
    } else {
      tbody.innerHTML = agents
        .map((a) => {
          const initiales = (a.agentNom || "AG")
            .split(" ")
            .map((p) => p[0])
            .join("")
            .toUpperCase()
    ...
    if (streamTbody) {
      if (!pointages || pointages.length === 0) {
        streamTbody.innerHTML =
          '<tr><td colspan="4" class="px-6 py-6 text-center text-slate-400">Aucun pointage enregistré aujourd\'hui.</td></tr>';
      } else {
        streamTbody.innerHTML = pointages
          .slice(0, 5)
          .map(
            (p) => `
                            <tr class="hover:bg-slate-50/60 transition-colors">
    ...
    try {
      ...
      if (Array.isArray(evals) && evals.length > 0) {
        const totalScore = evals.reduce(
          (sum, e) => sum + (e.scoreGlobal || e.scoreTotal || 8),
          0,
        );
    ...
    document.getElementById("personnelTableBody").innerHTML =
      '<tr><td colspan="4" class="px-6 py-4 text-center text-red-500">Erreur de chargement: ' +
      (e.message || "Inconnu") +
      "</td></tr>";
  }
}
...
async function loadPointagesLog() {
  try {
    ...
    const response = await apiFetch(
      `/employeur/pointages?date=${selectedDate}`,
    );
    ...
    if (pointages && pointages.length > 0) {
      ...
      if (pointages[0]) {
        document.getElementById("statOverviewDernier").textContent =
          pointages[0].agent_nom +
          (pointages[0].heure_sortie ? " (Sortie)" : " (Entree)");
      }
    } else {
    ...
    document.getElementById("pointageLogsTableBody").innerHTML =
      '<tr><td colspan="4" class="px-6 py-4 text-center text-red-500">Erreur: ' +
      (e.message || "Inconnu") +
      "</td></tr>";
  }
}
...
window.generateFichePdf = function (nom, poste, site, statut) {
  ...
  const statutColor =
    statut === "ACTIVE" || statut === "ACTIF" ? [16, 185, 129] : [245, 158, 11];
  ...
  doc.setFontSize(8);
  doc.setFont("helvetica", "normal");
  doc.text("SimpleTaff — Gestion du Personnel & Sécurité", W / 2, H - 5, {
    align: "center",
  });
...
window.printFiche = function (nom, poste, site, statut) {
  const initiales = nom
    ...
    .toUpperCase();
  const statutBg =
    statut === "ACTIVE" || statut === "ACTIF" ? "#10b981" : "#f59e0b";
  const now = new Date().toLocaleDateString("fr-FR", {
    day: "2-digit",
    month: "long",
    year: "numeric",
  ...
  try {
    ...
    if (evalAgentSelect) {
      evalAgentSelect.innerHTML = agents
        .map(
          (a) => `<option value="${a.agentId}">${a.agentNom || "N/A"}</option>`,
        )
        .join("");
    }
...
function renderSanctions(list) {
  ...
  tbody.innerHTML = list
    .map((s) => {
      ...
      if (isExpired) {
        badgeHTML =
          '<span class="badge bg-green-100 text-green-700 font-bold px-2 py-0.5 rounded-md text-xs">Terminé</span>';
      } else {
        const badgeClass =
          s.statut === "EN_COURS"
            ? "bg-amber-100 text-amber-700"
  ...
  try {
    ...
    if (container) {
      if (validNotifications.length === 0) {
        container.innerHTML =
          '<div class="text-center text-slate-400 text-sm py-4">Aucune notification</div>';
      } else {
        container.innerHTML = validNotifications
          .map(
            (n) => `
...
if (searchInput) {
  searchInput.addEventListener("input", function (e) {
    ...
    // Search Agents
    const agentsData =
      typeof window.allAgents !== "undefined" ? window.allAgents : [];
