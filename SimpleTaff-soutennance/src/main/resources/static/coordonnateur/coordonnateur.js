...
function showTab(name) {
  ...
  document.querySelectorAll(".sidebar-link").forEach((l) => {
    if (
      l.dataset.tab === name ||
      l.getAttribute("onclick")?.includes("'" + name + "'")
    )
...
function populateAgentZoneFilter() {
  ...
  if (select && select.options.length <= 1) {
    const zones = [
      ...new Set(window.allCoordAgents.map((a) => a.zoneNom).filter(Boolean)),
    ];
    select.innerHTML =
...
window.filterAgents = function () {
  const searchVal =
    document.getElementById("searchAgentInput")?.value?.toLowerCase()?.trim() ||
    "";
  const zoneVal = document.getElementById("filterAgentZoneSelect")?.value || "";
  const tbody = document.getElementById("agentsTableBody");
  if (!tbody) return;

  const activeList = window.allCoordAgents.filter(
    (a) =>
...
window.filterEnrolementAgents = function () {
  const pendingList = window.allCoordAgents.filter(
    (a) =>
      a.statut === "EN_ATTENTE_CONTRAT_SIGNE" ||
      a.statut === "EN_ATTENTE_FINALISATION_ADMIN",
  );
...
function renderCoordPointages(data) {
  ...
  if (!data || data.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="5" class="px-6 py-6 text-center text-slate-400">Aucun pointage pour cette date.</td></tr>';
...
window.filterCoordPointages = function () {
  const filterVal =
    document.getElementById("coordStructureFilter")?.value || "";
  const filtered = filterVal
    ? _coordPointages.filter((p) => p.structureCliente === filterVal)
    : _coordPointages;
...
async function loadPointages() {
  const tbody = document.getElementById("pointagesTable");
  const selectedDate =
    document.getElementById("coordPointageDate")?.value ||
    new Date().toISOString().slice(0, 10);
  try {
    const data = await apiFetch(
      `/coordonnateur/pointages?date=${selectedDate}`,
    );
...
async function loadPointageDates() {
  ...
  try {
    ...
    if (!dates || dates.length === 0) {
      container.innerHTML =
        '<span class="text-slate-400">Aucun jour enregistré pour le moment.</span>';
      return;
    }
...
async function loadZones() {
  ...
  try {
    ...
    if (!data || data.length === 0) {
      container.textContent = "Aucune zone configurée.";
      return;
    }
...
async function loadCoordMateriel() {
  ...
  try {
    ...
    if (data.length === 0) {
      if (tbodyRequests)
        tbodyRequests.innerHTML =
          '<tr><td colspan="8" class="p-6 text-center text-slate-400">Aucune demande enregistrée.</td></tr>';
      if (tbodyApproved)
        tbodyApproved.innerHTML =
          '<tr><td colspan="7" class="p-6 text-center text-slate-400">Aucun historique d\'approbation.</td></tr>';
    } else {
      const badgeMap = {
      ...
      if (tbodyRequests) {
        tbodyRequests.innerHTML = data
          .map((d) => {
            const coord = d.coordonnateurNom || "—";
            const dateDem = d.dateDemande
              ? d.dateDemande.replace("T", " ").substring(0, 16)
              : "—";
      ...
      const approvedDemands = data.filter(
        (d) =>
          d.statut === "APPROUVE" ||
          d.statut === "APPROUVEE" ||
          d.statut === "ACCORDE",
      ...
      if (tbodyApproved) {
        if (approvedDemands.length === 0) {
          tbodyApproved.innerHTML =
            '<tr><td colspan="7" class="p-6 text-center text-slate-400">Aucune demande acceptée pour le moment.</td></tr>';
        } else {
          tbodyApproved.innerHTML = approvedDemands
            .map((d) => {
              const coord = d.coordonnateurNom || "—";
              const dateDem = d.dateDemande
                ? d.dateDemande.replace("T", " ").substring(0, 16)
    ...
    if (tbodyRequests)
      tbodyRequests.innerHTML = `<tr><td colspan="8" class="p-3 text-center text-red-400">${e.message}</td></tr>`;
    if (tbodyApproved)
      tbodyApproved.innerHTML = `<tr><td colspan="7" class="p-3 text-center text-red-400">${e.message}</td></tr>`;
  }
  ...
  try {
    ...
    if (tbodyInventory) {
      if (materiels.length === 0) {
        tbodyInventory.innerHTML =
          '<tr><td colspan="7" class="p-6 text-center text-slate-400">Aucun matériel dans l\'inventaire de l\'entreprise.</td></tr>';
      } else {
        tbodyInventory.innerHTML = materiels
          .map((m) => {
            let statusBadge = "";
            const st = (m.statut || "").toUpperCase();
            if (st === "DISPONIBLE") {
    ...
    if (tbodyInventory)
      tbodyInventory.innerHTML = `<tr><td colspan="7" class="p-3 text-center text-red-400">${e.message}</td></tr>`;
  }
...
window.declarerIncidentMateriel = async function (id, libelle) {
  ...
  const details = prompt(
    `Motif & circonstances de l'incident terrain (ex: Écran fendu lors de l'intervention, radio égarée par l'agent X) :`,
    "",
  );
  ...
  try {
    ...
    alert(
      `Incident enregistré (${statut}) ! L'état du matériel a été mis à jour et l'alerte a été transmise à la direction.`,
    );
    loadCoordMateriel();
  } catch (e) {
    console.error(e);
...
async function loadCoordConges() {
  ...
  try {
    ...
    if (!data || data.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="4" class="p-6 text-center text-slate-400">Aucune demande enregistrée.</td></tr>';
      return;
    }
    const badgeMap = {
    ...
    tbody.innerHTML = data
      .map((d) => {
        ...
        if (isExpired) {
          statBadge =
            '<span class="badge bg-green-100 text-green-700 font-bold">Terminé</span>';
        } else {
...
async function loadOverview() {
  ...
  try {
    ...
    if (stats) {
      ...
      if (attendusEl)
        attendusEl.textContent = `Sur ${stats.agentsAttendus ?? 0} attendus`;

      document.getElementById("statAffectations").textContent =
        stats.absencesRetards ?? "—";
      document.getElementById("statPointages").textContent =
        stats.demandesMateriel ?? "—";
      ...
      if (chartContainer && labelsContainer && stats.zonesCoverage) {
        ...
        if (stats.zonesCoverage.length === 0) {
          chartContainer.innerHTML =
            '<div class="absolute inset-0 flex items-center justify-center text-slate-400 text-sm">Aucune donnée</div>';
        } else {
  ...
  try {
    ...
    if (tbodyPtRec) {
      if (todayPointagesList.length === 0) {
        tbodyPtRec.innerHTML =
          '<tr><td colspan="4" class="px-6 py-6 text-center text-slate-400 font-medium">Aucun pointage enregistré aujourd\'hui.</td></tr>';
      } else {
        tbodyPtRec.innerHTML = todayPointagesList
          .slice(0, 6)
          .map((p) => {
            const isEntree = (p.typePointage || "").toUpperCase() === "ENTREE";
            const badge = isEntree
    ...
    if (tbodyPtRec)
      tbodyPtRec.innerHTML = `<tr><td colspan="4" class="px-6 py-6 text-center text-red-400">${e.message}</td></tr>`;
  }
  ...
  try {
    ...
    if (tbodyAffRec) {
      ...
      if (activeAffectations.length === 0) {
        tbodyAffRec.innerHTML =
          '<tr><td colspan="4" class="px-6 py-6 text-center text-slate-400 font-medium">Aucun agent actuellement assigné.</td></tr>';
      } else {
        tbodyAffRec.innerHTML = activeAffectations
          .slice(0, 6)
          .map((a) => {
            const agentNom = a.agentNom || "Agent Inconnu";
            const initials =
            ...
            const pointageAjd = todayPointagesList.find(
              (p) => p.agentNom === agentNom,
            );
            let statusBadge =
              '<span class="badge bg-slate-100 text-slate-600 font-medium">Non pointé</span>';
            let timeInfo =
              '<div class="text-slate-500 text-[10px]">En attente de prise de poste</div>';
    ...
    if (tbodyAffRec)
      tbodyAffRec.innerHTML = `<tr><td colspan="4" class="px-6 py-6 text-center text-red-400">${e.message}</td></tr>`;
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const addMaterielRequestForm = document.getElementById(
    "addMaterielRequestForm",
  );
  if (addMaterielRequestForm) {
    addMaterielRequestForm.addEventListener("submit", async (e) => {
      ...
      try {
        const libelle = document
          .getElementById("reqMaterielLibelle")
          .value.trim();
        const categorie = document.getElementById("reqMaterielCategorie").value;
        const numeroSerie = document
          .getElementById("reqMaterielNumeroSerie")
          ?.value?.trim();
        const valeurAchatStr =
          document.getElementById("reqMaterielValeur")?.value;
        ...
        submitBtn.innerHTML =
          '<i class="fa-solid fa-paper-plane"></i> Transmettre à l\'Administration';
      }
    });
  }
  ...
  if (addCongeRequestForm) {
    addCongeRequestForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        const justifFile =
          document.getElementById("reqCongeJustifFile")?.files?.[0];
        ...
        document.getElementById("reqCongeJustifStatus").textContent =
          "Optionnel — certificat médical, justification officielle...";
...
function renderCoordEvaluations(list) {
  ...
  if (!list || list.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="6" class="p-3 text-center text-slate-400">Aucune évaluation enregistrée.</td></tr>';
    return;
  }
...
window.filterEvaluationsCoord = function () {
  const query =
    document.getElementById("searchEvaluationsCoord")?.value?.toLowerCase() ||
    "";
...
async function loadCoordDisciplinaire() {
  ...
  try {
    ...
    if (tbody)
      tbody.innerHTML =
        '<tr><td colspan="6" class="p-3 text-center text-red-500">Erreur de chargement des sanctions</td></tr>';
  }
}
...
function renderCoordSanctions(list) {
  ...
  if (!list || list.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="7" class="p-3 text-center text-slate-400">Aucune sanction enregistrée.</td></tr>';
    return;
  }
  tbody.innerHTML = list
    .map((s) => {
...
window.filterSanctionsCoord = function () {
  const query =
    document.getElementById("searchSanctionsCoord")?.value?.toLowerCase() || "";
  if (!window.allCoordSanctions) return;
  renderCoordSanctions(
    window.allCoordSanctions.filter((s) => {
...
window.switchPersonnelSubTab = function (tab) {
  ...
  if (tab === "actifs") {
    btnActifs.className =
      "pb-2 border-b-2 border-sky-600 text-sky-600 font-bold";
    btnEnrol.className =
      "pb-2 border-b-2 border-transparent text-slate-500 hover:text-slate-800 flex items-center gap-1.5";
    secActifs.classList.remove("hidden");
    secEnrol.classList.add("hidden");
  } else {
    btnActifs.className =
      "pb-2 border-b-2 border-transparent text-slate-500 hover:text-slate-800";
    btnEnrol.className =
      "pb-2 border-b-2 border-sky-600 text-sky-600 font-bold flex items-center gap-1.5";
  ...
  if (form) {
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        alert(
          "L'enrôlement a été finalisé avec succès ! L'agent est désormais ACTIF.",
        );
...
window.initAgentModernModules = function () {
    ...
    modalAddAgentForm.addEventListener("submit", async (e) => {
      ...
      if (birthInput && birthInput.value.trim()) {
        ...
        if (!validOptions.includes(birthInput.value.trim())) {
          alert(
            "Erreur : Veuillez sélectionner un Lieu de naissance valide dans la liste des villes de Côte d'Ivoire.",
          );
          birthInput.focus();
          return;
        }
      ...
      if (cityInput && cityInput.value.trim()) {
        ...
        if (!validOptions.includes(cityInput.value.trim())) {
          alert(
            "Erreur : Veuillez sélectionner une Ville valide dans la liste des villes de Côte d'Ivoire.",
          );
          cityInput.focus();
          return;
        }
      ...
      if (communeInput && communeInput.value.trim()) {
        ...
        if (!validOptions.includes(communeInput.value.trim())) {
          alert(
            "Erreur : Veuillez sélectionner une Commune valide dans la liste des communes de Côte d'Ivoire.",
          );
          communeInput.focus();
          return;
        }
      ...
      const payload = {
        ...
        telephoneSecondaire: document.getElementById(
          "mAgentTelephoneSecondaire",
        ).value,
        ...
        contactUrgenceTelephone: document.getElementById(
          "mAgentUrgenceTelephone",
        ).value,
  ...
  const folderCreateContractForm = document.getElementById(
    "folderCreateContractForm",
  );
    ...
    folderCreateContractForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        const payload = {
          ...
          structureClienteId:
            document.getElementById("fcStructureCliente").value,
  ...
  const folderRenewContractForm = document.getElementById(
    "folderRenewContractForm",
  );
  ...
  try {
    ...
    if (select) {
      const disponibles = (allMateriels || []).filter(
        (m) => m.statut === "DISPONIBLE" || m.statut === "ASSIGNE",
      );
...
window.zoomQr = function (fullName, qrData, zone) {
  document.getElementById("adminQrModalTitle").textContent =
    "Badge — " + fullName;
    ...
    if (container) {
      container.innerHTML =
        data.length > 0
          ? data
              .map(
                (n) => `<div class="p-2 border-b text-sm">${n.message}</div>`,
              )
              .join("")
...
if (searchInput) {
  searchInput.addEventListener("input", function (e) {
    ...
    // Search Agents
    const agentsData =
      typeof window.allCoordAgents !== "undefined" ? window.allCoordAgents : [];
    ...
    const affectationsData =
      typeof window.allCoordAffectations !== "undefined"
        ? window.allCoordAffectations
        : [];
    affectationsData.forEach((a) => {
      ...
      if (name.includes(q) || site.includes(q)) {
        results.push({
          ...
          tab: "affectations",
          action: () => {
            if (typeof showTab === "function") showTab("affectations");
            searchInput.value = "";
            searchDropdown.classList.add("hidden");
          },
        });
    ...
    const entreprisesData =
      typeof window.entreprises !== "undefined" ? window.entreprises : [];
    entreprisesData.forEach((ent) => {
      if ((ent.nom || "").toLowerCase().includes(q)) {
        results.push({
          ...
          tab: "entreprises",
          action: () => {
            if (typeof showTab === "function") showTab("entreprises");
            searchInput.value = "";
            searchDropdown.classList.add("hidden");
          },
        });
